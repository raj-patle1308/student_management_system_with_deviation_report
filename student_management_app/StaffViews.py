...
from student_management_app.models import CustomUser, Staffs, Courses, Subjects, Students, SessionYearModel, Attendance, AttendanceReport, LeaveReportStaff, FeedBackStaffs, StudentResult ,Timetable
...
def save_attendance_data(request):
    ...
    lecture_start = request.POST.get("lecture_start")  # New field
    lecture_end = request.POST.get("lecture_end")      # New field

    # Convert lecture_start and lecture_end to time objects if necessary
    lecture_start_time = time.fromisoformat(lecture_start)
    lecture_end_time = time.fromisoformat(lecture_end)
    ...
    try:
        # Save attendance data, including lecture_start and lecture_end
        attendance = Attendance(
            subject_id=subject_model,
            attendance_date=attendance_date,
            session_year_id=session_year_model,
            lecture_start=lecture_start_time,
            lecture_end=lecture_end_time
        )
        attendance.save()

        for stud in json_student:
            # Save attendance report for individual students
...
def view_timetable(request):
    ...
    return render(request, 'staff_template/timetable.html', {'timetables': timetables})